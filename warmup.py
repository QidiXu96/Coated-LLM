...
def extract_data(text):
    # Compile regex to find all instances of "Effective in theory:" followed by any characters until a period, newline, or end of the string
    pattern1 = re.compile(r'Effective in theory:.*?(?=\.\s|\n|$)', re.IGNORECASE)
    matches = pattern1.findall(text)
    
    if matches:
        first_match = matches[0]
        # Search for a word immediately following "Effective in theory:"
        pattern2 = re.search(r'Effective in theory: (\w+)', first_match)
...
def warmup(X_train, y_train, params, client, output_hdf5_path="embeddings.hdf5", pathway_dir="pathway", output_json_path="warm_up_CoT.json"):
    ...
    for i in X_train.index:
        ...
        predicted_answer = extract_data(chain_of_thoughts)
